...
class ProductDetailScreen extends StatefulWidget {
  ...
  @override
  _ProductDetailScreenState createState() => _ProductDetailScreenState(productid, totalprice);
}
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      const Text("Product Detail", textAlign: TextAlign.center, style: TextStyle(fontStyle: FontStyle.normal, fontSize: 21, fontWeight: FontWeight.bold)),
                      Padding(
                           ...
                           Column(
                             children: [
                                 Card(
                                    ...
                                    child: images.length == 0 || images.isEmpty ? ClipRRect(
                                 ...
                                 Padding(
                                     ...
                                     child: Align(
                                         ...
                                         child: Text(productname.toString() == "" || productname.toString() == "null" || productname.toString() == null ? ""  : productname, textAlign: TextAlign.start, style: TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.bold))),
                                 ),
                                 netweight == "" || netweight == null ? SizedBox() : Padding(
                                   padding: EdgeInsets.only(left: 20, right: 20),
                                   child: Align(
                                       alignment: Alignment.topLeft,
                                       child: Text(netweight.toString() == "" || netweight.toString() == null || netweight.toString() == "null" ? "" : "Netweight(gm): $netweight", style: TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.w400))),
                                 ),
                                 itemlength== "" || itemlength == null ? SizedBox() : Padding(
                                 padding: EdgeInsets.only(left: 20, right: 20),
                                 child: Align(
                                     alignment: Alignment.topLeft,
                                     child: Text(itemlength.toString() == "" || itemlength.toString() == null || itemlength.toString() == "null" ? "" : "Dimension(cm): $itemlength x $itemwidth x $itemheight", style: TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.w400))),
                                 ),
                                 SizedBox(height: 5),
                                 Padding(
                                 padding: EdgeInsets.only(left: 20, right: 20),
                                 child: Row(
                                     mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                     children: [
                                         Text("\u20B9 $rate", style: TextStyle(color: Colors.grey, fontSize: 14, decoration: TextDecoration.lineThrough)),
                                         Text("\u20B9 $offerprice", style: TextStyle(color: Colors.black, fontSize: 14)),
                                     ],
                                 )),
                                 groupprice == "" || groupprice == null ? SizedBox() : Padding(
                                     ...
                                     child: Card(
                                       ...
                                       child: Padding(
                                         ...
                                         child: Align(
                                             alignment: Alignment.topLeft,
                                             child: Text(groupprice.toString() == "" || groupprice.toString() == null || groupprice.toString() == "null" ? "" : "$groupprice", style: TextStyle(color: Colors.black, fontSize: 16, fontWeight: FontWeight.w400))
                                         ),
                                       ) ,
                                     )),
                                 SizedBox(height: 10),
                                 Padding(
                                   ...
                                   child: Align(
                                       alignment: Alignment.topLeft,
                                       child: Text(short_desc.toString() == "" || short_desc.toString() == null || short_desc.toString() == "null" ? "" : short_desc, style: TextStyle(color: Colors.black, fontSize: 15)))
                                 ),
                                 Padding(
                                 padding: EdgeInsets.only(left: 20, right: 20),
                                 child: Align(
                                     alignment: Alignment.topLeft,
                                     child: Text(long_desc.toString() == "" || long_desc.toString() == null || long_desc.toString() == "null" ? "" : long_desc, style: TextStyle(color: Colors.black, fontSize: 12)))
                                 ),
                                 SizedBox(height: 10),
                                 Padding(
                                       ...
                                       InkWell(
                                         onTap: (){
                                           if(proQty.toString() == "0"){
                                             showToast("Your product quantity is less than 1");
                                           }
                                           else{
                                             _addtocart(productid.toString(), offerprice.toString(), proQty.toString(), rate.toString());
                                           }
  ...
  Future _productdetails(String productid) async{
    ...
    if (response.statusCode == 200)
    {
      ...
      setState(() {
         ...
         images.addAll(json.decode(response.body)['Response']['details']['additional_images']);

         productimage = json.decode(response.body)['Response']['details']['product_image'].toString();
         productname = json.decode(response.body)['Response']['details']['product_name'].toString();
         netweight = json.decode(response.body)['Response']['details']['weight'].toString();
         short_desc = json.decode(response.body)['Response']['details']['short_description'].toString();
         long_desc = json.decode(response.body)['Response']['details']['long_description'].toString();

         proQty = json.decode(response.body)['Response']['quantity'];
         rate = json.decode(response.body)['Response']['details']['mrp'].toString();
         offerprice = json.decode(response.body)['Response']['details']['discount_price'].toString();
         amount = json.decode(response.body)['Response']['details']['mrp'].toString();
